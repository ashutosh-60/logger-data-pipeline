...
# 1. Thread-Isolated Context State
...
current_upstream_id: ContextVar[str] = ContextVar("current_upstream_id", default="")

# 2. Helpers to easily update or retrieve context
def set_pipeline_run_id(run_id: Optional[str] = None) -> str:
    """Set the pipeline execution ID, generating one only when missing."""
    existing_run_id = current_run_id.get()
    if run_id is None and existing_run_id:
...
def initialize_pipeline_context(run_id: Optional[str] = None) -> str:
    """Compatibility helper for callers that initialize lineage explicitly."""
    return set_pipeline_run_id(run_id)

# 3. The Autonomous Interceptor
class LineageFilter(logging.Filter):
    """Intercepts log records and injects active lineage context variables."""
    def filter(self, record: logging.LogRecord) -> bool:
        record.run_id = current_run_id.get()
        record.task_id = current_task_id.get()
        record.upstream_id = current_upstream_id.get() or "root"
        return True
